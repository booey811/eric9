...
def update_older_postcodes():
	"""goes through the Courier Pricing Board and Updates the minified postcdoe values (where possible)"""
	import eric.assets.monday as mon_objects
	import eric.utils as utils

	b = utils.get_board('courier_pricing')
	col_val = b.mon_board.get_column_value(id='status8')
	col_val.value = col_val.text = col_val.label = 'Do Now'
	items = [mon_objects.MondayItem(item) for item in b.mon_board.get_items_by_column_values(column_value=col_val)]
	for item in items:
		full_postcode = item.columns.sample_postcode.value.replace(" ", "")
		try:
			minified = parse_uk_postcode(full_postcode, False)
			item.columns.mini_postcode.value = minified[0]
			item.columns.pc_update.value = 'Complete'
		except Exception as e:
			item.add_update(f"Could Not Get Minified Postcode: {e}")
			item.columns.pc_update.value = 'Error'
			print(str(e))
		item.commit()
